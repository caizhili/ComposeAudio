...
public class AACEncode {
    ...
    public AACEncode() {
        ...
        final int kSampleRates[] = {8000, 11025, 22050, 16000, 44100, 48000};
        //比特率 声音中的比特率是指将模拟声音信号转换成数字声音信号后，单位时间内的二进制数据量，是间接衡量音频质量的一个指标
        final int kBitRates[] = {16000, 32000, 64000, 96000, 128000};

        //初始化   此格式使用的音频编码技术、音频采样率、使用此格式的音频信道数（单声道为 1，立体声为 2）
        MediaFormat mediaFormat = MediaFormat.createAudioFormat(
                mime, kSampleRates[3], 1);

        mediaFormat.setString(MediaFormat.KEY_MIME, mime);
        mediaFormat.setInteger(MediaFormat.KEY_AAC_PROFILE,
                MediaCodecInfo.CodecProfileLevel.AACObjectLC);
        //比特率 声音中的比特率是指将模拟声音信号转换成数字声音信号后，单位时间内的二进制数据量，是间接衡量音频质量的一个指标
        mediaFormat.setInteger(MediaFormat.KEY_BIT_RATE, kBitRates[2]);
...
//    public void startEncoder(String fromPath,String toPath){
//        try{
//            FileInputStream fileInputStream = FileFunction.GetFileInputStreamFromFile(fromPath);
//            FileOutputStream fileOutputStream = FileFunction.GetFileOutputStreamFromFile(toPath);
//            fileInputStream.
//        }catch (Exception e){
//            e.printStackTrace();
//        }
